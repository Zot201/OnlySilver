...
public class Dynamic {
	...
	public static <T> Construct<T> construct(final String clz) {
		return new Construct<>(Klas.<T>ofName(clz));
	}
	...
	public static abstract class Viable<T, U extends Viable<T, U>> extends Chain<T> {
		protected final ImmutableList<Supplier<TypeArg<?>>> args;
		...
		protected <V> U viaArg(Supplier<TypeArg<?>> arg) {
		...
		protected <V> U viaKlas(final Klas<V> type, final Supplier<V> arg) {
			return viaArg(new Supplier<TypeArg<?>>() { public TypeArg<?> get() {
				return new TypeArg<V>(type, arg.get());
			}});
		}
		...
		public U viaInt(final Supplier<Integer> arg) {
			return viaArg(new Supplier<TypeArg<?>>() { public TypeArg<?> get() {
				return new TypeArg<Integer>(Klas.INT, arg.get());
			}});
		}
	...
	public static class Invoke<T> extends Viable<T, Invoke<T>> {
		...
		private Invoke(Klas<?> clz, Supplier<?> obj, String method) {
			this(clz, obj, method, ImmutableList.<Supplier<TypeArg<?>>>of());
		}
	...
	public static class Construct<T> extends Viable<T, Construct<T>> {
		...
		private Construct(Klas<? extends T> clz) {
			this(clz, ImmutableList.<Supplier<TypeArg<?>>>of());
		}
		...
		public Extend<T> extend(Consumer<GeneratorAdapter> extend) {
			return new Extend<T>(this, extend, ImmutableList.<Consumer<KlastWriter<?>>>of());
		}
	...
	public static class Extend<T> extends Chain<T> {
		...
		@Override public T get() {
			List<TypeArg<?>> args = FluentIterable.from(parent.args)
					.transform(Suppliers.<TypeArg<?>>supplierFunction())
					.toList();
